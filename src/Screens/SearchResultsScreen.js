...
const SearchResultsScreen = () => {
    ...
    const isMatch = (e, query) => {
        return e.title.toLowerCase().includes(query.toString().toLowerCase())
            || e.desc.toLowerCase().includes(query.toString().toLowerCase())
            || e.location.toLowerCase().includes(query.toString().toLowerCase())
            || e.restrictions.toLowerCase().includes(query.toString().toLowerCase())
    }
    useEffect(() => {
        async function fetch() {
            getTMEvent(query).then(r => {
                let filtered_events = []
                for (const ev of r) {
        ...
        fetch().then(() => {
            getEvents().then((r) => {
                for (const de of r) {
                    if(!searchEvent.find(fe => fe.name === de.title) && isMatch(de, query)) {
                        searchEvent.push(de)
                    }
                }
    ...
    const searchHandlerChange = (value) => {
        setInput(value)
        setSearchEvent(searchEvent.filter(e => isMatch(e, input)))
    }

    const searchHandlerSubmit = (e) => {
        e.preventDefault();
        navigate(`/frydei/search/${e.target.search.value}`)
    }
    if(!searchEvent) {
        return null
    }
    ...
    return (
        <>
            <div style={{"paddingLeft": "175px", "paddingRight": "175px", "paddingTop": "25px"}}>
                ...
                <form action="" name="search-form" onSubmit={(e) => searchHandlerSubmit(iref.current.value)}>
